...
(function() {
    ...
    window.addEventListener("load", function() {
      document.getElementById("singleSearchForm").addEventListener("submit", function(event) {
        ...
        //change the url to /search/"subject"/"course"
        let subjectCode = document.getElementById("singleSubject").value.toUpperCase();
        let courseCode = document.getElementById("singleCourse").value;
        window.location.href = `/search/${subjectCode}/${courseCode}`;
        return;
      }, false);
    }, false);
